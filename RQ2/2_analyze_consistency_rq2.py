...
def calculate_icc(df_runs, metric='move_accuracy'):
    """
    Calculate Intraclass Correlation Coefficient (ICC).
    
    Args:
        df_runs: DataFrame with run data
        metric: Metric to calculate ICC for
    ...
    # Need to reshape data for ICC calculation
    # ICC requires long format: targets (articles) × raters (runs)
    # For now, we'll use article-level accuracy across runs
    
    # Load sentence-level data to get article-level scores
    results_dir = Path("evaluation_results") / f"{df_runs.iloc[0]['run_number']}_rq2"
    
    # This is a simplified ICC calculation on run-level accuracy
    # A more sophisticated version would use article-level or sentence-level data
    
    # For aggregate ICC, we can use a single-measure ICC
    # treating each run as a "rater" and the overall accuracy as the "target"
    
    # Simplified: Return None for now, indicate this needs article-level data
    print("  ℹ️  ICC calculation requires article-level reshaping (future enhancement)")
    return None
...
def analyze_condition(condition, dataset="test"):
    ...
    print(f"  Calculating ICC...", end=" ")
    icc_results = calculate_icc(df_runs, 'move_accuracy')
    if icc_results:
        print("✓")
    else:
        print("⊘ (skipped)")
...
def create_summary_report(results, output_dir):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for _, row in df_stats.iterrows():
            ...
            f.write(f"  Range: [{row['min']:.4f}, {row['max']:.4f}] (span: {row['range']:.4f})\n")
            f.write(f"  Median: {row['median']:.4f}\n")
            f.write(f"  IQR: {row['iqr']:.4f}\n")
            f.write(f"  Normality: {'Normal' if row['is_normal'] else 'Non-normal'} (Shapiro-Wilk p={row['shapiro_p']:.4f})\n")
            f.write("\n")
...
def save_outputs(results, output_dir):
    ...
    results['df_runs'].to_csv(
        output_dir / f"{condition}_all_runs.csv",
        index=False
    )
    
    # Save descriptive statistics
    results['df_stats'].to_csv(
        output_dir / f"{condition}_descriptive_stats.csv",
        index=False
    )
...
def main():
    ...
    try:
        ...
        print(f"  Move Accuracy: {move_row['mean']:.3f} ± {move_row['sd']:.3f} (CV: {move_row['cv']:.2f}%)")
