...
Output format matches position-based evaluation needs:
- Each sentence gets a position number (1, 2, 3, ...)
...
def prepare_gold_standard_for_article(xml_path, output_path):
    ...
    for position, sentence in enumerate(article['sentences'], start=1):
        # Handle multi-tag sentences (if they exist in format "1a, 2b")
        step_code = sentence['step']
        tags = [tag.strip() for tag in step_code.split(',')] if ',' in step_code else [step_code]
